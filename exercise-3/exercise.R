...
words <- c("Fashion","Biology","Psychiatry","Design","Red","Doors")
words
...
word_bin <- function(words, letter1, letter2) {
  first <- words[substring(words,1,1)]
  is_between <- first[first > letter2 && first <= letter1]
  words[c(is_between)]
}
word_bin(words,'g','m')
